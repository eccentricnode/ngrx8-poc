...
import { State, animalsAdapter, initialState } from './animals.reducer';
import * as AnimalsSelectors from './animals.selectors';

describe('Animals Selectors', () => {
  const ERROR_MSG = 'No Error Available';
  const getAnimalsId = it => it['id'];
  ...
  describe('Animals Selectors', () => {
    ...
    it('getSelected() should return the selected Entity', () => {
      const result = AnimalsSelectors.getSelected(state);
